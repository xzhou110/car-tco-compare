...
- **Cost breakdown** — one **vertical stacked column per car** (taller = costs more), column height ∝ total spend (scaled to the max), segments = the 7 cost components in `CATEGORY_ORDER` colored by `CATEGORY_COLORS`, stacked **bottom-to-top** in the same order in every column. Total above each column; car name + slot-color dot below; **category legend at the bottom** (consistent with the cumulative chart). The column row scrolls horizontally within the panel when many cars don't fit a narrow width. **Hovering a segment** shows a cursor-following tooltip with the component label, its exact dollar value, and its share of that car's gross spend (the hovered segment brightens).
- **Cumulative chart** — inline SVG, per-slot gradient area fill under each line, white-cored data points, dashed gridlines. **Hovering** snaps a dashed vertical crosshair to the nearest year, enlarges that year's data points, and shows a tooltip listing every car's running total at that year (costliest first, with a `· purchase` / `· sale` note at the endpoints).
- **Chart tooltip** (`ChartTooltip`) — shared floating overlay for both charts: a non-interactive (`pointer-events: none`) absolutely-positioned card inside a `position: relative` `.chart-area` wrapper, themed with the standard surface/line tokens. It flips its anchor away from the cursor near the right/top edges so it never clips out of view; pointer (not mouse) events make it work under touch as well.
