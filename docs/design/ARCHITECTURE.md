...
```
  ...
  │  • localStorage + shareable-link state       │      │  • twice-daily cron → digest    │
...
- **Two stores, two cadences:** the app snapshot refreshes **weekly** (`refresh-listings.yml`); the alert cache refreshes **twice daily** in Supabase (`alerts.yml`). Both stay inside Auto.dev's free 1,000-calls/mo tier (RAV4-only twice daily + one ~100-call weekly full pull ≈ <600/mo).
...
```
                ┌─ twice-daily cron (GitHub Actions, alerts.yml) ──────────┐
...
| `refresh-listings.yml` | cron Mon 16:00 UTC (+ manual) | Rebuild `listings.json` (3 models, Auto.dev), commit, **self-deploy** (a `GITHUB_TOKEN` commit can't trigger `deploy.yml`) |
| `alerts.yml` | cron 15:00 + 21:00 UTC (+ manual) | Refresh alert cache, send confirmations, send each subscriber their TCO-ranked digest |
