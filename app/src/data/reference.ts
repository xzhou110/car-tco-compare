...
export const SEGMENT_LIST: { key: SegmentKey; label: string }[] = (Object.keys(REFERENCE.rates) as SegmentKey[]).map((k) => ({ key: k, label: REFERENCE.rates[k].label }));
export const REGION_LIST: { key: string; label: string }[] = Object.keys(REFERENCE.regions).map((k) => ({ key: k, label: REFERENCE.regions[k].label }));
